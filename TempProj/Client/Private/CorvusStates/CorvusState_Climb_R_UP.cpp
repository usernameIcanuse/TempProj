...
void CCorvusState_Climb_R_UP::Start()
{
	...
	m_iAnimIndex = m_pModelCom.lock()->Get_IndexFromAnimName("Corvus_SD_Ladder_Climb_R_UP");
	m_pModelCom.lock()->CallBack_AnimationEnd += bind(&CCorvusState_Climb_R_UP::Call_AnimationEnd, this, placeholders::_1);
}
...
void CCorvusState_Climb_R_UP::Call_AnimationEnd(_uint iEndAnimIndex)
{
	if (!Get_Enable())
		return;
...
void CCorvusState_Climb_R_UP::OnStateStart(const _float& In_fAnimationBlendTime)
{
	...
	m_pThisAnimationCom.lock()->CallBack_NextChannelKey +=
		bind(&CCorvusState_Climb_R_UP::Call_NextKeyFrame, this, placeholders::_1);
...
void CCorvusState_Climb_R_UP::Call_NextKeyFrame(const _uint& In_KeyIndex)
{
	if (!Get_Enable())
		return;
