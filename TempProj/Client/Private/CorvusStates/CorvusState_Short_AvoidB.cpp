...
GAMECLASS_C(CCorvusState_Short_AvoidB);
CLONE_C(CCorvusState_Short_AvoidB, CComponent)

HRESULT CCorvusState_Short_AvoidB::Initialize_Prototype()
{
	__super::Initialize_Prototype();
	return S_OK;
}

HRESULT CCorvusState_Short_AvoidB::Initialize(void* pArg)
{
...
void CCorvusState_Short_AvoidB::Start()
{
	__super::Start();
	
	m_pModelCom = m_pOwner.lock()->Get_Component<CModel>();
	m_iAnimIndex = m_pModelCom.lock()->Get_IndexFromAnimName("Corvus_SD_AvoidShort_B_NEW");
	m_pModelCom.lock()->CallBack_AnimationEnd += bind(&CCorvusState_Short_AvoidB::Call_AnimationEnd, this, placeholders::_1);
}

void CCorvusState_Short_AvoidB::Tick(_float fTimeDelta)
{
...
void CCorvusState_Short_AvoidB::LateTick(_float fTimeDelta)
{
...
void CCorvusState_Short_AvoidB::OnDisable()
{

}

void CCorvusState_Short_AvoidB::OnStateStart(const _float& In_fAnimationBlendTime)
{
	__super::OnStateStart(In_fAnimationBlendTime);

	

	if (Get_OwnerCharacter().lock()->Get_PreState().lock() == Get_Owner().lock()->Get_Component<CCorvusState_Short_AvoidB>().lock())
	{
...
void CCorvusState_Short_AvoidB::Call_AnimationEnd(_uint iEndAnimIndex)
{
	if (!Get_Enable())
		return;
...
void CCorvusState_Short_AvoidB::Free()
{

}

void CCorvusState_Short_AvoidB::OnDestroy()
{
	if (m_pModelCom.lock())
		m_pModelCom.lock()->CallBack_AnimationEnd -= bind(&CCorvusState_Short_AvoidB::Call_AnimationEnd, this, placeholders::_1);
}

_bool CCorvusState_Short_AvoidB::Check_AndChangeNextState()
{
	...
	if (m_pModelCom.lock()->Get_CurrentAnimation().lock()->Get_CurrentChannelKeyIndex() >= 24)
	{
		if (Check_RequirementDashState())
		{
			...
			Get_OwnerPlayer()->Change_State<CCorvusState_Short_AvoidB>();
			return false;
		}
	}
...
_bool CCorvusState_Short_AvoidB::Check_RuquireMnetRepeatAvoidkState()
{
